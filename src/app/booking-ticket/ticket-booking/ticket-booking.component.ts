...
export class TicketBookingComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,public trainTicketService:TrainTicketService) {
    this.initateForm();
  }
  ...
  public ngOnInit(): void {
    ...
    // let trainTicketMockService = new TrainTicketService();
    this.res = this.trainTicketService.get();
  }


  public onSubmit(): void {
    const userName: string = this.bookingForm.controls.name.value;
    const requiredSeats: number = parseInt(this.bookingForm.controls['count'].value);
    ...
    else if (requiredSeats > 0 && requiredSeats <=7) {
